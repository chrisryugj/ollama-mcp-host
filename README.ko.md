...
> #### 💻 gemma4:e4b 최소 사양
>
> AI 모델을 *실제로 돌리는* PC(전략 A를 쓴다면 그 서버 한 대)의 사양입니다.
>
> | | 최소 | 권장 | 비고 |
> |---|---|---|---|
> | **디스크** | 12 GB | 20 GB | 모델 파일이 약 **9.6 GB**. 여유 공간 포함 |
> | **RAM** | 8 GB | 16 GB | CPU만으로 돌릴 때 모델을 메모리에 올리기 위함 |
> | **GPU** | 없어도 됨 | 통합메모리(Apple Silicon) 또는 VRAM 6 GB↑ | GPU가 있으면 훨씬 빠름. 없으면 CPU로 동작(느림) |
>
> `e4b`는 메모리 효율이 좋은 구조라(필요한 파라미터만 활성화) **실제 실행 중 점유량은 약 3.3 GB** 수준입니다. 그래서 디스크 9.6 GB에 비해 RAM/VRAM 부담은 작은 편입니다.
> *(실측: Apple M5 · RAM 32 GB · GPU 로드 3.3 GB · 컨텍스트 32K에서 쾌적하게 동작)*
>
> 💡 **전략 A를 쓰면 이 사양은 서버 한 대에만 필요합니다.** 나머지 직원 PC들은 모델을 안 돌리고 네트워크로 접속만 하므로, `ollama-mcp-host`(Node)만 켜질 정도의 평범한 사무용 PC면 충분합니다.
